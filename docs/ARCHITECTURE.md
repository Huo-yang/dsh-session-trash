...
软删除保留日志原位和工作区关联，只写回收站索引。永久删除按如下步骤执行：

1. 显式永久删除在同一个写队列槽中完成登记与清理，恢复不能插入两步之间。
2. 持久化原路径及 `deletionPhase: staged`，再搬入暂存目录。
3. 持久化 `deletionPhase: deleting`，再递归删除。
4. 删除成功后清理工作区关联，移除索引并落盘。
